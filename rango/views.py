...
def user_login(request):
    if request.method == 'POST':
        ...
        # use Django's machinery to attempt to see if the combination is valid. A user object is returned if it is.
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            user = user_form.save()
            # We hash the password using the set_password method
...
def add_category(request):
    ...
    if request.method == 'POST':
        # How does CategoryForm accept POST
